...
class GuardManager {
    ...
    async isGuardCompatible() {
        ...
        const safeVersion = __classPrivateFieldGet(this, _GuardManager_safeContract, "f").safeVersion;
    ...
    async getGuard() {
        ...
        return __classPrivateFieldGet(this, _GuardManager_safeProvider, "f").getStorageAt(safeContract.getAddress(), __classPrivateFieldGet(this, _GuardManager_slot, "f"));
    }
    async encodeEnableGuardData(guardAddress) {
        ...
        return safeContract.encode('setGuard', [(0, types_1.asHex)(guardAddress)]);
    }
    async encodeDisableGuardData() {
        const safeContract = await this.isGuardCompatible();
        const currentGuard = await this.getGuard();
        this.validateGuardIsEnabled(currentGuard);
        return safeContract.encode('setGuard', [(0, types_1.asHex)(constants_1.ZERO_ADDRESS)]);
    }
}
